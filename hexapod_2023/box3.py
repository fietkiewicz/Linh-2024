...
def init_and_run():
    ...
    while viewer.is_alive and i < N:
        ...
        if phase == 0 and data.joint('pitch_mid_right').qpos[0] <= float(angle_1.get()):
            ...
            phase = 4
            printDebug(phase, data)
        elif phase == 4 and data.joint('pitch_mid_left').qpos[0] <= float(angle_1.get()):
            phase = 5
            printDebug(phase, data)
        elif phase == 5 and data.joint('yaw_mid_left').qpos[0] <= float(angle_2.get()):
            phase = 6
            printDebug(phase, data)
        elif phase == 6 and data.joint('pitch_mid_left').qpos[0] >= float(angle_3.get()):
            phase = 7
            printDebug(phase, data)
        elif phase == 7 and data.joint('yaw_mid_left').qpos[0] >= float(angle_4.get()):
            phase = 0
            printDebug(phase, data)
...
init_btn = Button(window, text="Init and Run", width=30, command=init_and_run).grid(row=8,column=0, columnspan=4, pady=2)
save_btn = Button(window, text="Save", width=12, command=save_setting).grid(row=9, column=0, columnspan=2, padx=5, pady=2)
load_btn = Button(window, text="Load", width=12, command=load_setting).grid(row=9, column=2, columnspan=2, padx=5, pady=2)
