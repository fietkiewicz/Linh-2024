...
def init_and_run():
    ...
    for geom in tree.iter('geom'):
        if geom.get('name') in ["floor", "L_foot", "L_toe_mesh_1", "L_toe_mesh_2", "L_toe_mesh_3", "L_toe_mesh_4", "L_toe_mesh_5", 
                                "R_foot", "R_toe_mesh_1", "R_toe_mesh_2", "R_toe_mesh_3", "R_toe_mesh_4", "R_toe_mesh_5"]:
...
friction_label = Label(window, text="Friction").grid(row=0, column=0, pady=2)
friction = Entry(window, width=7)
friction.grid(row=0, column=2, pady=2)

tracking = BooleanVar()
camera_label = Label(window, text="Camera").grid(row=1, column=0, pady=2, sticky=W)
track_camera = Radiobutton(window, text="Tracking", variable=tracking, value=True).grid(row=1, column=2, sticky=W)
fixed_camera = Radiobutton(window, text="Fixed", variable=tracking, value=False).grid(row=1, column=3, sticky=W)

speed_label = Label(window, text="Speed").grid(row=2, column=0)
elevation_label = Label(window, text="Elevation").grid(row=2, column=1)
distance_label = Label(window, text="Distance").grid(row=2, column=2)
trackbody_label = Label(window, text="Trackbody").grid(row=2, column=3)

speed = Entry(window, width=7)
speed.grid(row=3, column=0, pady=2)
elevation = Entry(window, width=7)
elevation.grid(row=3, column=1, pady=2)
distance = Entry(window, width=7)
distance.grid(row=3, column=2, pady=2)
trackbody = Entry(window, width=7)
trackbody.grid(row=3, column=3, pady=2)

live = BooleanVar()
live_camera = Radiobutton(window, text="Live", variable=live, value=True).grid(row=4, column=0, sticky=W)
video_camera = Radiobutton(window, text="Video", variable=live, value=False).grid(row=4, column=1, sticky=W)
video_directory = Entry(window, width=10)
video_directory.grid(row=4, column=2)
step = Entry(window, width=7)
step.grid(row=4, column=3)

init_btn = Button(window, text="Init and Run", command=init_and_run, width=28).grid(row=5, column=0, columnspan=4)
save_btn = Button(window, text="Save", width=14, command=save_setting).grid(row=6, column=0, columnspan=2)
load_btn = Button(window, text="Load", width=14, command=load_setting).grid(row=6, column=2, columnspan=2)
