...
def init_and_run():
    ...
    hip_extensor = [0, 0.7, 0.5, 0, 0, 0, 0, 0, 0, 0]
    hip_flexor = [0.2, 0.2, 0, 0, 0, 0, 0, 0, 0, 0]
    ankle_extensor = [0.5, 1, 0, 0, 0, 0, 0, 0, 0, 0]
    ankle_flexor = [0, 0, 0.5, 0.2, 0, 0, 0, 0, 0, 0]
    knee_flexor = [0, 0.5, 0.5, 0, 0, 0, 0, 0, 0, 0]
    ...
    while viewer.is_alive and i < N:
        ...
        data.ctrl[2] = knee_flexor[phase]
        data.ctrl[4] = ankle_extensor[phase]
        data.ctrl[5] = ankle_flexor[phase]
        data.ctrl[6] = hip_extensor[(phase + 5) % 10]
        data.ctrl[7] = hip_flexor[(phase + 5) % 10]
        data.ctrl[8] = knee_flexor[(phase + 5) % 10]
        data.ctrl[10] = ankle_extensor[(phase + 5) % 10]
        data.ctrl[11] = ankle_flexor[(phase + 5) % 10]
